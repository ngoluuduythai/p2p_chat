...
use libp2p::core::{
    ...
    PeerId, StreamMuxer, Transport, muxing::StreamMuxerBox,
};
...
use futures::{stream::SelectAll, AsyncRead, AsyncWrite, Stream, StreamExt, future::Ready, FutureExt};
...
impl Future for QuicUpgrade {
    type Output = Result<(PeerId, StreamMuxerBox), io::Error>;
...
impl Transport for QuicTransport
{
    ...
    fn listen_on(&mut self, addr: Multiaddr) -> Result<ListenerId, TransportError<Self::Error>> {
        ...
        // let listener = self
        //     .do_listen(listener_id, socket_addr)
                            ...
                            Ok(IfEvent::Down(inet)) => {
                                ...
                                if self.socket_addr().is_ipv4() == ip.is_ipv4() {
                                    let socket_addr =
                                        SocketAddr::new(ip, self.socket_addr().port());
                                    let ma = socketaddr_to_multiaddr(&socket_addr);
                                    println!("Expired listen address: {}", ma);
...
impl Stream for Listener 
{
    ...
    // type Item = Result<ListenerEvent<T::Stream>, io::Error>;
...
//     use crate::helper::generate_ed25519;
